import argparse
from ..api import S3Config, S3API
from .. import __prog__,__version__
import os


def get_args(prog, version):
    ...
    msessages.add_argument(
        "--silent",
        help="Не отображать собщений результатов работы",
        action="store_true",
    )

    s3storage = parser.add_argument_group(
        "Параметры S3-хранилища",
        description=f'Все параметры в этом разделе можно установить через переменные окружения с префиксом "{s3c.default_env_prefix}", например параметр HOST устанавливается переменной окружения "{s3c.default_env_prefix}HOST".',
    )
    ...
    s3storage.add_argument(
        "--protocol",
        dest="protocol",
        help='Протокол (http или https) для работы с S3-хранилищем (по умолчанию: "%(default)s")',
        default=s3c.protocol,
    )
    s3storage.add_argument(
        "--access_key",
        dest="access_key",
        help="Ключ доступа",
        default=s3c.access_key,
        required=not s3c.access_key,
    )
    s3storage.add_argument(
        "--secret_key",
        dest="secret_key",
        help="Сектретный ключ",
        default=s3c.secret_key,
        required=not s3c.secret_key,
    )
    s3storage.add_argument("--host", help="Имя хоста или IP-адрес", default=s3c.host, required=not s3c.host)
    s3storage.add_argument(
        "--port",
        type=str,
        help="Порт S3-хранилища (по умолчанию устанавливается в зависимости от протокола",
        default=s3c.port,
        required=False,
    )
    s3storage.add_argument(
    ...
    other.add_argument(
        "--save_empty_dir",
        action="store_true",
        help="Хранить пустые директории",
        default=False,
    )

    cmd: argparse._SubParsersAction[argparse.ArgumentParser] = parser.add_subparsers(
        title="Комманды",
        required=True,
        dest="command",
        description="""\
    ...
    ls.add_argument(
        ...
        "--key_prefix",
        dest="key_prefix",
        help="Не обязательный префикс для отображения объектов хранилища",
        required=False,
    )

    _ = "Отправка файла или директории в хранилище."
    up: argparse.ArgumentParser = cmd.add_parser(
        "upload",
        help=_,
        description=_,
        epilog="""
    ...
    up.add_argument(
        "-f",
        "--file",
        help="Имя директории или файла для отравки в храничище (относительно рабочей директории)",
        required=True,
    )
    ...
    up.add_argument(
        "-c",
        "--cut_file_path",
        help="Не обязательный вырезаемый из файлового пути префикс",
    )
    ...
    dn.add_argument(
        "-a",
        "--add_file_path",
        help="Добавляемый путь в локальной файловой системе к имени ключа",
    )
    dn.add_argument(
        "-c",
        "--cut_key_prefix",
        help="Вырезаемый из ключей префикс для формирования имени файла",
    )

    _ = "Удаление ключей"
    rm: argparse.ArgumentParser = cmd.add_parser(
        "delete",
        help=_,
        description=_,
    )
    rm.add_argument(
        ...
        "--key_prefix",
        dest="key_prefix",
        help="Префикс для удаления объектов в хранилище",
        required=True,
    )
...
def main():
    args = get_args(prog=__prog__, version=__version__)
    s3c = S3Config()
    ...
    if "list" == args.command:
        ...
        print(
            line_delimiter.join(
                [line_format.format(**i) for i in s3.list(args.key_prefix)]
            )
        )
        ...
        s3.upload(
            filename=args.file,
            add_key_prefix=args.add_key_prefix,
            cut_file_path=args.cut_file_path,
        )
        ...
        s3.download(
            ...
            cut_key_prefix=args.cut_key_prefix,
        )
