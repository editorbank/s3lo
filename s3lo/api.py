...
class S3Config():
  ...
  def init_from_environments_variables(self, prefix:str=None):
    if not prefix:
...
class S3API():
  ...
  def as_dir(self, name :str = None) -> str:
    if not name: name = ''
    if name != '' and not name.endswith('/'): name = name + '/'
    return name

  def is_dir(self, name :str = None) -> bool:
    if not name: name = ''
    ...
    for _key in self.keys_list(prefix=key_prefix):
      ...
      if not self.is_dir(_key):
        ...
        if self.save_empty_dir:
          if not self.silent: print(f' Download key "{_key}" as empty dir "{_filename}" ...')
          Path(_filename).mkdir(parents=True, exist_ok=True)

  def get(self, filename, default='') -> str:
    key = self.bucket.get_key(filename)
    return key.get_contents_as_string().decode(encoding=self._cat_encoding) if key else default
  ...
  def upload(self, filename, add_key_prefix:str = None, cut_file_path:str = None):
    if not add_key_prefix : add_key_prefix =''
    if not cut_file_path : cut_file_path =''
    for i in self.fs_list(filename):
      _key = add_key_prefix + _cut_prefix(i,cut_file_path)
      if not self.is_dir(i):
        if not self.silent: print(f'Upload key "{_key}" from "{i}" ...')
